...
frontend_splash = FRONTEND_BUILD_DIR / "static" / "logo.png"

if frontend_splash.exists():
    try:
        shutil.copyfile(frontend_splash, STATIC_DIR / "logo.png")
    except Exception as e:
        logging.error(f"An error occurred: {e}")
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Fawer AI + Ollama)
